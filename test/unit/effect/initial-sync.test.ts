...
describe("Initial Sync - Folder and Cleanup Handlers", () => {
	...
	describe("folderSync during initial sync", () => {
		test("creates folder data directory", async () => {
			await Effect.runPromise(Effect.provide(folderSync("/test/books/", "Fiction"), TestLayer));
		...
		test("processes nested folder paths correctly", async () => {
			await Effect.runPromise(Effect.provide(folderSync("/test/books/Fiction/", "SciFi"), TestLayer));
	...
	describe("folderCleanup during initial sync", () => {
		test("removes orphan folder directory", async () => {
			await Effect.runPromise(Effect.provide(folderCleanup("/test/books/", "OldFolder"), TestLayer));
	...
	describe("bookCleanup during initial sync", () => {
		test("removes orphan book directory", async () => {
			await Effect.runPromise(Effect.provide(bookCleanup("/test/books/Fiction/", "deleted.epub"), TestLayer));
			expect(mockFs.rmCalls).toHaveLength(1);
	...
	describe("sync flow simulation", () => {
		test("processes multiple folders sequentially", async () => {
			...
			for (const folder of folders) {
				await Effect.runPromise(Effect.provide(folderSync("/test/books/", folder), TestLayer));
			}
		...
		test("cleanup then create for folder replacement", async () => {
			await Effect.runPromise(Effect.provide(folderCleanup("/test/books/", "OldFolder"), TestLayer));
			await Effect.runPromise(Effect.provide(folderSync("/test/books/", "NewFolder"), TestLayer));
		...
		test("logs operations for each handler", async () => {
			await Effect.runPromise(Effect.provide(folderSync("/test/books/", "Fiction"), TestLayer));
			await Effect.runPromise(Effect.provide(bookCleanup("/test/books/Fiction/", "old.epub"), TestLayer));
