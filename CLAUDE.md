...
If you change `ui/styles/*`, `ui/gridnav/*`, or the renderer, run `bun run build:ui` and commit the regenerated `static/` artifacts. `bun run test:all` runs `build:ui:check` (a `git diff --exit-code static/` freshness gate) before the test suites.
...
**To change the browser template:** edit `src/render/feed-html.ts` (markup) and/or `ui/styles/*` (CSS). Run `bun run dev:ui` for a live HMR preview against real cassettes. Run `bun run build:ui` to regenerate `static/style.css` + `static/main.js`, then restart the container (or `POST /resync`) to regenerate every `index.html`. Never hand-edit `static/style.css` or `static/main.js`.
